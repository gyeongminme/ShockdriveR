...
public class Player : MonoBehaviour
{
    ...
    public float Cur_Time = 0; // 피격 쿨타임
    ...
    {
        if (collision.gameObject.tag == "campfire" && Cool())
        {
            Debug.Log("불에 10 피해를 입음.");
            Cur_Hp = Hurt(10,Cur_Hp);
            hp_bar.value = (float)Cur_Hp / (float)Max_Hp;
        }
    }
    ...
    public float Hurt(float damage,float Cur_Hp)
    {
